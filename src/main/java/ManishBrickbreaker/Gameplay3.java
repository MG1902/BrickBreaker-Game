...
public class Gameplay3 extends JPanel implements KeyListener, ActionListener 
{
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{	
			...
			if(new Rectangle(ballposX2, ballposY2, 20, 20).intersects(new Rectangle(playerX, 9*y/10, 30, 8)))
			{
				ballYdir2 = -ballYdir2;
				if(ballXdir2>0)
					ballXdir2 = -ballXdir2;
			}
			else if(new Rectangle(ballposX2, ballposY2, 20, 20).intersects(new Rectangle(playerX + 30, 9*y/10, 40, 8)))
			{
				ballYdir2 = -ballYdir2;

			}
			else if(new Rectangle(ballposX2, ballposY2, 20, 20).intersects(new Rectangle(playerX + 70, 9*y/10, 30, 8)))
			{
				ballYdir2 = -ballYdir2;
				if(ballXdir2<0)
					ballXdir2 = -ballXdir2;
			...
			A: for(int i = 0; i<map.map.length; i++)
			{
				...
				{				
					if(map.map[i][j] > 0)
					{
						...
						Rectangle brickRect1 = rect;
						Rectangle brickRect2 = rect;

						if(ballRect1.intersects(brickRect1)||ballRect2.intersects(brickRect2))
						{					
							if(map.map[i][j] == 1)
							{
							...
							if(ballRect1.x <= brickRect1.x - 20 || ballRect1.x  >= brickRect1.x + brickRect1.width)	
							{
								ballXdir1 = -ballXdir1;
							}
							...
							if(ballRect2.x <= brickRect2.x - 20 || ballRect2.x  >= brickRect2.x + brickRect2.width)	
							{
								ballXdir2 = -ballXdir2;
							}
							// when ball hits top or bottom of brick
